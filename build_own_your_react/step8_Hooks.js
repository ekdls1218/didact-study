...
function useState(initial) {
  ...
  actions.forEach((action) => {
    // 이전 렌더와 이번 렌더 사이에 발생한 setState들 실행
...
function reconcileChildren(wipFiber, elements) {
  ...
  while (index < elements.length || oldFiber != null) {
    // index가 elements의 길이보다 작거나 오래된 fiber가 있을 때 동안 반복
    ...
    if (sameType) {
      // oldFiber,element가 있고, 둘이 타입이 같을 때
      newFiber = {
        type: oldFiber.type,
        props: element.props,
        dom: oldFiber.dom,
